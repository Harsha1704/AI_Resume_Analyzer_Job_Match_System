...
# ═══════════════════════════════════════════════════════════════
    ...
    if jobs:
        jobs_s = sorted(jobs,key=lambda x:x.get("score",0) if isinstance(x,dict) else 0,reverse=True)
        titles = [j.get("job_title","").replace("-"," ").title() if isinstance(j,dict) else str(j) for j in jobs_s]
        scores = [round(j.get("score",0),1) if isinstance(j,dict) else 0 for j in jobs_s]
        ...
        for i,job in enumerate(jobs_s[:6]):
            t  = job.get("job_title","").replace("-"," ").title() if isinstance(job,dict) else str(job)
            sc = round(job.get("score",0),1) if isinstance(job,dict) else 0
...
# ═══════════════════════════════════════════════════════════════
    ...
    if jobs:
        ...
        jobs_s=sorted(jobs,key=lambda x:x.get("score",0) if isinstance(x,dict) else 0,reverse=True)[:8]
        titles=[j.get("job_title","").replace("-"," ").title() if isinstance(j,dict) else str(j) for j in jobs_s]
        scores_j=[round(j.get("score",0),1) if isinstance(j,dict) else 0 for j in jobs_s]
...
# ═══════════════════════════════════════════════════════════════
    ...
    jobs_s = sorted(jobs,key=lambda x:x.get("score",0) if isinstance(x,dict) else 0,reverse=True)[:5]
    job_lines="\n".join([f"  {i+1}. {j.get('job_title','').replace('-',' ').title()} — {round(j.get('score',0),1)}%" for i,j in enumerate(jobs_s) if isinstance(j,dict)])
